...
def try_register(user_form, user_profile_form=None):
    ...
    # if django.utils.translation.get_language() == "sl-si":  # TODO: remove - use reverse()
    #    activation_url = "aktiviraj-racun/kljuc="
    #else:

    activation_url = "activate-account/key="

    activation_url = settings.SITE_URL + settings.URL_PREFIX + activation_url + key

    file = open(settings.STATIC_DIR + "email/blusers/email_verification_" +
                django.utils.translation.get_language() + ".txt", "r")
    message_txt = file.read()
    file.close()
    message_txt = message_txt % (activation_url, settings.SITE_URL)
    message_txt = unicode(message_txt, 'utf-8')

    file = open(settings.STATIC_DIR + "email/blusers/email_verification_" +
                django.utils.translation.get_language() + ".html", "r")
    message_html = file.read()
    file.close()
    message_html = message_html % (activation_url, activation_url, settings.SITE_URL)
    message_html = unicode(message_html, 'utf-8')
    ...
    if settings.DEBUG:
        ...
        send_email(settings.EMAIL_FROM, [new_user.email], None, subject, message_txt, message_html)
...
def successfully_registered(request):
    context = {
        'title': _("Successfully registered"),
        'site_title': settings.GLOBAL["site_title"],
    }

    return render(request, "blusers/successfully-registered.html", context)


@api_view(['POST'])
def mobile_lost_password(request):
    ...
    try:
        if data and "email" in data:
            try:
                ...
                return JsonResponse({"status": "email_invalid", 'message': "Email is invalid"})

            try:
                bluser = BlocklogicUser.objects.get(email=data["email"])
                if bluser.type == GOOGLE:
                    return JsonResponse({"status": "google_login", 'message': "Try login with google"})
                ...
                return JsonResponse({"status": "user_does_not_exist", 'message': "User does not exist"})
    except:
        pass

    return JsonResponse({"status": "something_went_wrong", 'message': "Something went wrong"})
