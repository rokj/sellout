...
class BlocklogicUserForm(forms.ModelForm):
    ...
    email = forms.EmailField(label=_("E-mail"), required=True, max_length=75,
                             help_text=_("Required field. 30 characters or fewer. Letters, digits and @/./+/-/_ characters only."),
...
class UserProfileForm(forms.ModelForm):
    # website = forms.URLField(label=_("Website"), required=False, help_text=_("Personal website"))
    company_name = forms.CharField(label=_("Company name"), required=False, help_text=_("Name of the company"))
    company_address = forms.CharField(label=_("Company address"), required=False, help_text=_("Name of the company"))
    company_postcode = forms.CharField(label=_("Company postcode"), required=False)
    company_postname = forms.CharField(label=_("Company postname"), required=False)
    company_city = forms.CharField(label=_("Company city"), required=False)
    company_country = forms.ModelChoiceField(queryset=Country.objects.all().order_by("name"), required=False, empty_label=None)
    company_tax_id = forms.CharField(label=_("Company tax ID"), required=False)
    company_tax_payer = forms.ChoiceField(label=_("Compay tax payer"), choices=TAX_PAYER_CHOICES, widget=forms.Select(), required=False, initial="not_tax_payer")
    company_website = forms.CharField(label=_("Website of the company"), required=False)

    class Meta:
        model = UserProfile
        fields = ("company_name", "company_address", "company_postcode", "company_country", "company_tax_id", "company_website", "company_city", "company_tax_payer")